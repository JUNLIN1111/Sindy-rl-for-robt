...
if __name__ == '__main__':
    ...
    trial_dir = 'D:\\ray_results\go2-walk\\b1-walk_sindy_freq=10\\dyna_sindy_178ac_00003_3_2025-02-28_17-33-15'
    #trial_dir = 'D:\\ray_results\go2-walk\\PPO_BaseEnsembleSurrogateEnv_2025-02-27_22-08-278fvkvoq9'
    # load models, data, and env
    go2_dyn, go2_rew, go2_data, go2_policy, go2_env = get_models(
                                                                        trial_dir,
                                                                        check_idx= 190, # 1350, for overfit
                                                                        return_policy=True,
                                                                        )
    go2_unbounded_policy = RLlibPolicyWrapper(go2_policy.algo.get_policy(), mode='policy')


    # evaluate baseline
    go2_baseline_obs, go2_baseline_acts, go2_baseline_rews = rollout_env(
                                                                        go2_env.real_env, 
                                                                        policy=go2_policy, 
                                                                        n_steps = 15000, 
                                                                        verbose=True
                                                                        )
    baseline_perf = np.median([r.sum() for r in go2_baseline_rews])


    def fit_go2(env, dyn_model, policy, unbounded_policy, 
    ...
    # and select best one
    go2_res_dict, go2_normal_traj_data = fit_go2(go2_env, go2_dyn, 
                                                go2_policy, go2_unbounded_policy, 
                                                hyper_mesh,
                                                n_dyn_rollout=5000,
                                                use_median=False,
                                                poly_deg=3)

    go2_sparse_policy = go2_res_dict['best_policy']

    # example of saving the policy
    with open('go2_sparse_policy.pkl', 'wb') as f:
        pickle.dump(go2_sparse_policy, f)

    go2_sparse_policy.set_mean_coef_()
    go2_eval_obs, go2_eval_acts, go2_eval_rews = rollout_env(go2_env.real_env, 
                                                                policy=go2_sparse_policy, 
                                                                n_steps = 15000, verbose=True)
    perf = np.median([r.sum() for r in go2_eval_rews])
