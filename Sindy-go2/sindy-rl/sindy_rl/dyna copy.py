...
from sindy_rl.try_cropolicy import create_dim_reducer,train_dim_reducer,reduce_dimensionality
...
class DynaSINDy(BaseDynaSINDy):
    ...
    def __init__(self, config, logger=None, init_drl=True):
        ...
        '''
        n_control = 12 #这里注意一下，不是所有都是12.为了方便直接12了
        
        #加载一下环境咯
        env = self.config['real_env']
        pi = RandomPolicy(low=-1*np.ones(n_control), 
        ...
        #与环境交互100次，训练encoder
        for i in range(100):
            trajs_obs, trajs_acts, trajs_rews = rollout_env(env, pi,20000)#这里我也直接设了
            obs_list.append(trajs_obs)

        train_data = torch.tensor(np.array(obs_list), dtype=torch.float32)
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=32, shuffle=True)

        reducer = create_dim_reducer(latent_dim=24)#降到24维
        self.trained_model = train_dim_reducer(reducer, train_loader, epochs=50)
'''
    ...
    def collect_data(self, buffer, env, pi, **rollout_kwargs):
        ...
        '''
        train_data = torch.tensor(np.array(trajs_obs), dtype=torch.float32)
        obs_loader = torch.utils.data.DataLoader(train_data, batch_size=32, shuffle=True)

        # 处理 DataLoader，转换为 Tensor 再输入到 reduce_dimensionality
        all_obs = []
        for batch in obs_loader:
            reduced_obs = reduce_dimensionality(self.trained_model, batch)  # 这里 batch 是 Tensor
            all_obs.append(reduced_obs)

        # 将降维后的数据转换回 list
        trajs_obs = np.concatenate(all_obs, axis=0).tolist()
