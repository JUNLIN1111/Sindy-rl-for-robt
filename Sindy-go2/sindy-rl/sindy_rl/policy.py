...
import numpy as np
from sindy_rl import crocoddyl_policy
...
class CrocoddylPolicy(BasePolicy):
    def __init__(self):
        self.anymal_controller = crocoddyl_policy.data()
        print("crocoddyl policy running")

    def compute_action(self, obs):
        return self.anymal_controller.collect_Policy(obs)
...
class RandomPolicy(BasePolicy):
    ...
    def __init__(self,action_space = None, low=None, high=None, seed=0):
        ...
        self.magnitude = 8.0  # 1.0,这里改了，不然幅度太小了
...
class SparseEnsemblePolicy(BasePolicy):
    ...
    def compute_action(self, obs):
        ThetaX = self.feature_library.transform(obs)# 计算 Θ(X)
        u = self.optimizer.coef_ @ ThetaX # 计算 u = Xi @ Θ(X)
